...
class UtilisateurController extends Controller
{
    ...
    /**
     * Show eleveurs download page
     */
    public function eleveursDownload()
    {
        $this->checkDirectionAccess();

        $cooperatives = Cooperative::with('membresActifs')->orderBy('nom_cooperative')->get();
        
        // Statistiques des éleveurs
        $stats = [
            'total_eleveurs' => MembreEleveur::count(),
            'eleveurs_actifs' => MembreEleveur::where('statut', 'actif')->count(),
            'eleveurs_inactifs' => MembreEleveur::where('statut', 'inactif')->count(),
            'eleveurs_supprimes' => MembreEleveur::where('statut', 'suppression')->count(),
            'cooperatives_avec_eleveurs' => Cooperative::whereHas('membresActifs')->count(),
        ];

        return view('direction.eleveurs.download', compact('cooperatives', 'stats'));
    }

    /**
     * Download eleveurs list
     */
    public function downloadEleveurs(Request $request)
    {
        $this->checkDirectionAccess();

        $request->validate([
            'format' => 'required|in:csv,excel',
            'cooperative_id' => 'nullable|exists:cooperatives,id_cooperative',
            'statut' => 'nullable|in:actif,inactif,suppression',
        ...
        $eleveurs = $query->orderBy('nom_complet')->get();

        // Préparer les données
        $data = [];
        $data[] = [
            'Matricule Coopérative',
            'Nom Coopérative', 
            'Nom Complet Éleveur',
            'Email',
            'Téléphone',
        ...
        foreach ($eleveurs as $eleveur) {
            $data[] = [
                $eleveur->cooperative ? $eleveur->cooperative->matricule : 'N/A',
                $eleveur->cooperative ? $eleveur->cooperative->nom_cooperative : 'N/A',
                $eleveur->nom_complet,
                $eleveur->email,
                $eleveur->telephone,
                $eleveur->adresse,
                $eleveur->numero_carte_nationale,
                ucfirst($eleveur->statut),
                $eleveur->created_at->format('d/m/Y'),
                $eleveur->raison_suppression ?? ''
            ];
        }

        // Générer le fichier selon le format
        if ($request->format === 'csv') {
            return $this->generateCSV($data, 'eleveurs_' . date('Y-m-d_H-i-s') . '.csv');
        } else {
            return $this->generateExcel($data, 'eleveurs_' . date('Y-m-d_H-i-s') . '.xlsx');
        }
    }
